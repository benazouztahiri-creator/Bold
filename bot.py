#!/usr/bin/env python3
"""
Falcon AI v5.2 - Independent Strategies Only
==============================================
✅ Each strategy works independently
✅ No break after first signal
✅ All 4 strategies can fire together
...
class FalconPro:
    ...
    def analyze(self, symbol):
        """✅ كل استراتيجية تشتغل لوحدها - مفيش break"""
        results = []
        ...
        # ✅ كل الاستراتيجيات تشتغل - اللي ينجح يتبعت
        s1 = strat_divergence(df_15m, symbol)
        if s1 and s1['confidence'] >= MIN_CONFIDENCE:
            s1['symbol'] = symbol
            s1['expiry_time'] = (datetime.now() + timedelta(minutes=10)).strftime('%Y-%m-%d %H:%M:%S')
            results.append(s1)
        
        s2 = strat_breakout(df_15m, symbol)
        if s2 and s2['confidence'] >= MIN_CONFIDENCE:
            s2['symbol'] = symbol
            s2['expiry_time'] = (datetime.now() + timedelta(minutes=10)).strftime('%Y-%m-%d %H:%M:%S')
            results.append(s2)
        
        s3 = strat_ema(df_15m, symbol)
        if s3 and s3['confidence'] >= MIN_CONFIDENCE:
            s3['symbol'] = symbol
            s3['expiry_time'] = (datetime.now() + timedelta(minutes=10)).strftime('%Y-%m-%d %H:%M:%S')
            results.append(s3)
        
        s4 = strat_fibo(df_5m, df_1h, symbol)
        if s4 and s4['confidence'] >= MIN_CONFIDENCE:
            s4['symbol'] = symbol
            s4['expiry_time'] = (datetime.now() + timedelta(minutes=10)).strftime('%Y-%m-%d %H:%M:%S')
            results.append(s4)
