...
===============================
✅ Multi-timeframe confirmation (H1/H4/M15/M5)
...
def detect_structure(df):
    ...
    highs_20 = h[-20:]; lows_20 = l[-20:]
    
    swing_high = max(highs_20); swing_low = min(lows_20)
    ...
    if trend == 'UP' and recent_high > swing_high:
        bos = True
    elif trend == 'DOWN' and recent_low < swing_low:
        bos = True
    
    if trend == 'UP' and recent_low < swing_low:
        choch = True
    elif trend == 'DOWN' and recent_high > swing_high:
        choch = True
    
    return {'bos': bos, 'choch': choch, 'trend': trend, 'swing_high': swing_high, 'swing_low': swing_low}
...
def detect_liquidity_sweep(df):
    ...
    if current > swing_high * 1.001: return True
    if current < swing_low * 0.999: return True
    
    return False

def find_order_blocks(df):
    """✅ find Order Blocks (Supply/Demand)"""
    ...
    for i in range(5, len(c)-1):
        if c[i] < c[i-1] and c[i] < c[i+1]:
            demand_blocks.append({'low': l[i], 'high': h[i], 'close': c[i]})
        if c[i] > c[i-1] and c[i] > c[i+1]:
            supply_blocks.append({'low': l[i], 'high': h[i], 'close': c[i]})
    
    nearest_demand = demand_blocks[-1] if demand_blocks else None
    nearest_supply = supply_blocks[-1] if supply_blocks else None
...
def analyze_institutional(symbol):
    ...
    # Structure analysis on higher timeframe
    ...
    # ✅ CHoCH + Liquidity Sweep = Reversal
        ...
        if struct_h1['trend'] == 'UP' and demand:
            direction = 'BUY'
            setup_type = 'CHoCH + Liq Sweep (Reversal)'
            confidence = 0.82
...
def main():
    db = Database()
    logger.info("Falcon Institutional Strategy")
        ...
        try:
            ...
            for trade in db.get_expired():
                ...
                if df is not None and len(df) > 0:
                    ...
                    logger.info(f"{'WIN' if result=='WIN' else 'LOSS'} {trade['symbol']}: {pnl:+.2f}% | {trade.get('setup_type','')}")
